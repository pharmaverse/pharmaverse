...
The pharmaverse recommendation is for code to be hosted under an existing GitHub organisation - ‘OpenPharma’ (github.com/openpharma). While originally started by Roche/Genentech, this organisation now contains packages that are open-source collaborations between Roche/Genentech, Janssen, Novartis, Altran and Moorfields Eye Hospital (NHS). OpenPharma forms a neutral home for open-source Pharma-specific repos, and places no governance or burden on projects that ask to use this organisation beyond being relevant to Pharma. The admins of this org would include pharmaverse council members.
